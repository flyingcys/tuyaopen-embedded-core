...
OPERATE_RET PrintLogV(LOG_LEVEL logLevel, char *pFile, uint32_t line, char *pFmt, va_list ap)
{
    ...
    if (pLogManage->ms_level == FALSE) {
        ...
        cnt = snprintf(pLogManage->log_buf + len, pLogManage->log_buf_len - len, "[%02d-%02d %02d:%02d:%02d %s %s][%s:%d] ",
        ...
        cnt = snprintf(pLogManage->log_buf + len, pLogManage->log_buf_len + len, "[%02d-%02d %02d:%02d:%02d:%d %s %s][%s:%d] ",
 ...
 *
 * This function sets the color configuration for a specific log level, including the display mode, font color, and background color.
 *
 ...
 * @return NONE
 */
void tal_log_color_set(const TAL_LOG_LEVEL_E level, TAL_LOG_DISPLAY_MODE_E display_mode, TAL_LOG_FONT_COLOR_E font_color,
                        TAL_LOG_BACKGROUND_COLOR_E background_color)
{
    if (!pLogManage) {
        return;
                                    ...
                                    TAL_LOG_BACKGROUND_COLOR_E background_color, const char *pFmt, ...)
{
    ...
    if (pLogManage->log_color.enable_color) {
        cnt = snprintf(pLogManage->log_buf, pLogManage->log_buf_len, "\033[%d;%d;%dm",
                        display_mode, font_color, background_color);
        if (cnt <= 0) {
